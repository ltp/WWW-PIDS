...
#my @r = $p->GetNewClientGuid();
#my @r = $p->GetDestinationsForRoute( routeNo => 1 );
#my @r = $p->GetMainRoutes( );
#my @r = $p->GetMainRoutesForStop( stopNo => 3101 );
#my @r = $p->GetRouteStopsByRoute( routeNo => '1', isUpDirection => 1);
#my @r = $p->GetListOfStopsByRouteNoAndDirection( routeNo => 3, isUpDirection => 0);
my @r = $p->GetNextPredictedArrivalTimeAtStopsForTramNo( tramNo => 59 );
...
#my @r = $p->GetStopInformation( stopNo => 3101 );
